...
const char titleBBoxWin[30] = "Volume Bounding Box";
...
// windows
static int modelWindow, tfWindow, bBoxWindow;
...
static bool button_pressedBBoxWin = false;
...
GLint modelWinWidth=512, modelWinHeight=512,
  ...
  bBoxWinWidth=400, bBoxWinHeight=300;
...
static void mouseBBoxWin(GLint button, GLint state, GLint x, GLint y);
static void motionBBoxWin(GLint x, GLint y);
static void keyboardBBoxWin(unsigned char key, int x, int y);
...
void init(void) 
{
  ...
  if (write_in_file) {
    ...
    info_file << "* Num Tets: " << vol->getCurTets() << " / " << num_tets << " ; Num Vertices: " << num_verts << endl;
...
void reshapeBBoxWin(int w, int h)
{
  glutSetWindow(bBoxWindow);
  glViewport(0, 0, bBoxWinWidth=w, bBoxWinHeight=h);
}
...
/// Display Debug Text (bbox)

void displayDebugBBox(void)
{
  glColor3f(0.0, 0.0, 0.0);

  glWrite(1.04, -0.16, "Slices");

  GLfloat minX = vol->bB.getMinX();
  GLfloat minY = vol->bB.getMinY();
  GLfloat maxX = vol->bB.getMaxX();
  GLfloat maxY = vol->bB.getMaxY();

  ostringstream oss;
  oss << "Current BBox: ( " << minX << ", " << minY << " ), ( "
      << maxX << ", " << maxY << " ) " << endl;
  glWrite(-0.05, 1.12, (char*)oss.str().c_str());

  uint pLine = vol->bB.getPickedLine();

  if (pLine != 0) {
    glColor3f(0.7, 0.3, 0.1);
    // write the current line coordinate
  }
}
...
void mouseBBoxWin(GLint button, GLint state, GLint x, GLint y)
{
  if ((button == GLUT_LEFT_BUTTON) && (state == GLUT_DOWN))
    {
      vol->bB.pick(x, y);
      button_pressedBBoxWin = true;
      glutPostRedisplay();
    }
  if ((button == GLUT_LEFT_BUTTON) && (state == GLUT_UP))
    {
      vol->bB.update(x, y);
      vol->bB.set_picked_null();
      button_pressedBBoxWin = false;
      glutPostRedisplay();
    }
...
void motionBBoxWin(GLint x, GLint y)
{
  if (button_pressedBBoxWin) {
    vol->bB.update(x, y);
    glutSetWindow(bBoxWindow);
    glutPostRedisplay();
  }
...
void keyboardBBoxWin(unsigned char key, int x, int y)
{
  switch(key) {
  case 'r': case 'R':
    /*
    ...
    vol->readFile(volume_name, fType, dimX, dimY, dimZ);
    ...
    if (clear_white) glClearColor(1.0, 1.0, 1.0, 0.0);
...
int main(int argc, char** argv)
{
  ...
  while (argc > 2) {
	  ...
	  if ( strcmp(argv[argc-2], "-rot") == 0) {

		  rotX = (GLfloat)atoi(argv[argc-1]);
		  rotY = (GLfloat)atoi(argv[argc-1]);
		  rotZ = (GLfloat)atoi(argv[argc-1]);
		  argc -= 2;

	  } else if ( strcmp(argv[argc-2], "-x") == 0) {

		  rotX = (GLfloat)atoi(argv[argc-1]);
		  argc -= 2;

	  } else if ( strcmp(argv[argc-2], "-y") == 0) {

		  rotY = (GLfloat)atoi(argv[argc-1]);
		  argc -= 2;

	  } else if ( strcmp(argv[argc-2], "-z") == 0) {
  ...
  //-- bBox Window --
  glutInitWindowSize(bBoxWinWidth, bBoxWinHeight);
  glutInitWindowPosition(600, 500);

  bBoxWindow = glutCreateWindow(titleBBoxWin);

  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
  gluOrtho2D(BBOX_ORTHO_MIN, BBOX_ORTHO_MAX, BBOX_ORTHO_MIN, BBOX_ORTHO_MAX);
  glMatrixMode(GL_MODELVIEW);
  glLoadIdentity();
  ...
  glutMotionFunc(motionBBoxWin);
  glutMouseFunc(mouseBBoxWin);
  glutKeyboardFunc(keyboardBBoxWin);
  glutReshapeFunc(reshapeBBoxWin);
  glutDisplayFunc(displayBBoxWin);
